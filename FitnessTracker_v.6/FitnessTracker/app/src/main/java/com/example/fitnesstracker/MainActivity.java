...
import com.example.fitnesstracker.httpRequests.UpdateRequest;
...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Log.d(TAG, "ToString User: " + userInput);
    ...
    private void setupVars() {
        ...
        int lastIdSpace = userInput.lastIndexOf(" ");
        user_id = userInput.substring(lastIdSpace + 1);
    ...
    private void disconnectWithMiFit() {
        ...
        Log.w(TAG, "You was disconnecting from FitnessTracker");
    }
...
//                .addDataType(TYPE_SLEEP_SEGMENT, FitnessOptions.ACCESS_READ).build();
    ...
    private void getGoogleFitData() {
        int steps = getDataSteps();
        int pulse = getDataHeartRate();
        String sleep = getDataSleep();

        indicator.setSteps(steps);
        indicator.setPulse(pulse);
        indicator.setSleep(sleep);
        indicator.setUser_id(user_id);

        Log.w(TAG, "INDICATOR " + indicator.toString());
        new Thread(() -> updateRequest.updateIndicatorsRequest(indicator)).start();
    }
    ...
    private int getDataSteps() {
        ...
        if (listStepsRate.size() == 0) {
            ...
            tvStepCount.setText(String.valueOf(listStepsRate.get(listStepsRate.size() - 1)));
            return listStepsRate.get(listStepsRate.size() - 1);
        }
    }
    ...
    private int getDataHeartRate() {
        ...
        if (listHeartRate.size() == 0) {
            ...
            return listHeartRate.get(listHeartRate.size() - 1);
        }
    }
        ...
        sessionsClient.readSession(sessionReadRequest)
                .addOnSuccessListener(response -> {
                    for (Session session : response.getSessions()) {
                        ...
                        //Обновление данных
                        String sleep = (String.valueOf(hours) + "h " + String.valueOf(minutes) + "m");
                        tvSleep.setText(sleep);
                    }
                });

        //берем последнюю сессию
        if (!listStageSleep.isEmpty()) {
            HashMap<String, Integer> lastSession = listStageSleep.get(listStageSleep.size() - 1);

            long sumSleepingTime = 0;
            //считаем минуты по последней сессии
            for (Map.Entry<String, Integer> entry : lastSession.entrySet()) {
                sumSleepingTime += entry.getValue();
            }
